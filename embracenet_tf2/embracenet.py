...
class EmbraceNet(tf.keras.Model):
  ...
  def __init__(self, input_size_list, embracement_size=256, bypass_docking=False, **kwargs):
    """
  ...
  def call(self, input_list, availabilities=None, selection_probabilities=None):
    ...
    selection_probabilities = tf.math.divide(selection_probabilities, probabilty_sum)  # [batch_size, num_modalities]
    ...
    # embrace
    modality_indices = tf.random.categorical(tf.math.log(selection_probabilities), num_samples=self.embracement_size)  # [batch_size, embracement_size]
    modality_toggles = tf.one_hot(modality_indices, depth=num_modalities, axis=-1, dtype=tf.dtypes.float32)  # [batch_size, embracement_size, num_modalities]

    embracement_output_stack = tf.math.multiply(docking_output_stack, modality_toggles)  # [batch_size, embracement_size, num_modalities]
